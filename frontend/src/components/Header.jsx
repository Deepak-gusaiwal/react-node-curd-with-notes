    ...
    <header className={`flex items-center ${className}`}>
      <Container className=" flex gap-2 justify-between h-full p-0">
        ...
        <ul className={`menu duration-300  ${showMenu ? "active" : ""}`}>
          <li>
            <Link onClick={handdleShowMenu} to="/">Home</Link>
           </li>
        </ul>
        {/* contact button */}
          ...
          {loggedIn ? (
           ...
           <Button
            ...
            <Button className="" varient={{}} to="/signup">
              <span className="md:flex hidden">Signup</span> <IoPersonAdd className="text-white " />
            </Button>
            <Button className="" to="/login">
            <span className="md:flex hidden">login</span><MdLogin className="text-white " />
            </Button>
            </>
          )}
        </div>
